...
def testing():
    ...
    n_week = 45
    wk_ahead = 4
    model_type = model_types[3]  # Change
    error_total = {}
    predictions_total = {}
    real_values_total = {}
    ...
    for region in regions:
        for n_week in range(21, 46):
            ...
            seq_model = inputAttentionED.Seq2SeqModel(seqs.shape[1], seqs.shape[-1], RNN_DIM, wk_ahead)  # Change
            seq_model.load_state_dict(torch.load(path_model, map_location=torch.device(device)))
            # Testing the model
            seq_model.eval()
            # Allys is for two encoder models
            predictions_no_scaled, df_e, df_at = seq_model(seqs, mask_seq, allys, get_att=True)  # Change
