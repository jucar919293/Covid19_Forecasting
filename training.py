...
from seq2seqModels.models import InputEncoderDecoder,\
                                 ...
                                 InputEncoderAttentionDecoder, Input2EncoderDecoder, InputEncoderDecoderHidden4
...
version_models = ['SimpleForm/', 'Windowed/', 'WindowedHidden/', 'WindowedTemporal/', 'Tests/', 'Paper/']
aproaches = ['ED', 'InputEDHidden', 'InputETemporalD']
...
num_seqs = 7  # num_seqs >= 5
Tx = [15, 10, 5, 20, 25]  # T will be from [5, 10, 15, 20, 25]
...
def training_process():
    ...
    for T in Tx:
        stride = 1
        total_weeks = 49
        n_min_seqs = 7  # n_min_seqs >= num_seqs
        max_val_week = total_weeks - wk_ahead + 2  # changed +1 to +2
        ...
        for region in regions:
            ...
            for ew, ew_str in zip(weeks[min_val_week:max_val_week], weeks_strings[min_val_week-1:max_val_week-1]):
                ...
                # Saving the model and plot
                    path_model = model_path_save + region + '_' + ew_str + '_' + str(T) + '_' + str(theta) + '.pth'
                    torch.save(seqModel.state_dict(), path_model)
                    name_image = region + '_' + ew_str + '_' + aproach + '_' + str(T) + '_' + str(theta) + '.png'
